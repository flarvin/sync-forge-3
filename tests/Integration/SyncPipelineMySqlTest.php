...
final class SyncPipelineMySqlTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if (!is_string($dsn) || $dsn === '') {
            self::markTestSkipped('Set TEST_MYSQL_DSN to run MySQL integration tests.');
        }

        $parts = parse_url($dsn);
        if ($parts === false) {
            self::fail('Invalid TEST_MYSQL_DSN format.');
        }
        ...
        $this->connection->executeStatement('CREATE TABLE products (external_id VARCHAR(64) PRIMARY KEY, name VARCHAR(255) NOT NULL, price INT NOT NULL) ENGINE=InnoDB');
    }
     ...
     * @throws Exception
     */
    public function testMySqlExecutorUpsertAndDelete(): void
    {
        ...
        $resultV1 = $syncForge->for(MySqlProductStub::class)
            ->key(['external_id'])
            ->source([
        ...
        $resultV2 = $syncForge->for(MySqlProductStub::class)
            ->key(['external_id'])
            ->source([
    ...
    private function buildSyncForge(Connection $connection): SyncForge
    {
        $metadata = new EntityMetadata(
            entityClass: MySqlProductStub::class,
            tableName: 'products',
        ...
        $metadataProvider = new InMemoryEntityMetadataProvider([
            MySqlProductStub::class => $metadata,
        ]);
